<x-public-layout 
    ...
    description="Find answers to common questions about Cronjobs.to. Learn about scheduling, monitoring, alerts, pricing, security, and more."
    keywords="cronjobs faq, cron job questions, scheduler help, job monitoring faq, uptime monitoring questions"
>
    <!-- Hero Section -->
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div id="faq-container" class="space-y-8">
                @foreach($faqs as $categoryIndex => $category)
                    <section id="category-{{ Str::slug($category['category']) }}" class="faq-category scroll-mt-24" data-category="{{ strtolower($category['category']) }}">
                        <div class="flex items-center gap-3 mb-6">
                            <div class="w-10 h-10 rounded-xl flex items-center justify-center
                                @if($category['category'] === 'Getting Started') bg-accent-500/10
                                @elseif($category['category'] === 'Scheduling') bg-orange-500/10
                                @elseif($category['category'] === 'Monitoring & Alerts') bg-purple-500/10
                                @elseif($category['category'] === 'Security & Reliability') bg-emerald-500/10
                                @else bg-blue-500/10
                                @endif">
                            ...
                            <h2 class="text-2xl font-bold text-midnight-50">{{ $category['category'] }}</h2>
                        </div>

                        <div class="space-y-3">
                            @foreach($category['questions'] as $index => $qa)
                                <div class="faq-item group" data-question="{{ strtolower($qa['q']) }}" data-answer="{{ strtolower($qa['a']) }}">
                                    <button type="button" 
                                        ...
                                        <span class="font-semibold text-midnight-50 text-base pr-4">{{ $qa['q'] }}</span>
                                    ...
                                    <div id="faq-answer-{{ $categoryIndex }}-{{ $index }}" class="faq-answer overflow-hidden transition-all duration-300" style="max-height: 0;">
                                        <div class="px-5 pt-4 pb-5 bg-midnight-900 border-x border-b border-midnight-800 rounded-b-xl">
                                            <p class="text-midnight-300 leading-relaxed">{{ $qa['a'] }}</p>
            ...
            <div class="card p-10 relative overflow-hidden">
                ...
                <div class="relative">
                    ...
                    <div class="flex flex-col sm:flex-row justify-center gap-4">
                        ...
                        <a href="{{ route('docs') }}" class="btn-secondary inline-flex items-center justify-center gap-2 px-6 py-3">
                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <script type="application/ld+json">
    {
        ...
        "mainEntity": [
            @foreach($faqs as $categoryIndex => $category)
                @foreach($category['questions'] as $qaIndex => $qa)
            {
                "@@type": "Question",
                "name": "{{ addslashes($qa['q']) }}",
                "acceptedAnswer": {
                    "@@type": "Answer",
                    "text": "{{ addslashes($qa['a']) }}"
                }
