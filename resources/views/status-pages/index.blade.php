<x-app-layout>
    <x-slot name="title">{{ __('app.status_pages') }}</x-slot>

    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-midnight-50">{{ __('app.status_pages') }}</h1>
                <p class="text-sm text-midnight-400 mt-1">{{ __('app.status_pages_desc') }}</p>
            </div>
            <a href="{{ route('status-pages.create') }}" class="btn-primary">
                <svg class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                </svg>
                {{ __('app.new_status_page') }}
            </a>
        </div>
    </x-slot>
    ...
    <div class="card p-12 text-center">
        ...
        <h3 class="text-lg font-semibold text-midnight-100 mb-2">{{ __('app.no_status_pages_yet') }}</h3>
        <p class="text-midnight-400 mb-6 max-w-md mx-auto">
            {{ __('app.status_pages_empty_desc') }}
        </p>
        <a href="{{ route('status-pages.create') }}" class="btn-primary">
            <svg class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
            </svg>
            {{ __('app.create_first_status_page') }}
        </a>
    </div>
    @else
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="card p-6 hover:border-midnight-700 transition-colors">
            <div class="flex items-start justify-between mb-4">
                ...
                @if($page->is_public)
                <span class="badge-success">{{ __('app.public') }}</span>
                @else
                <span class="badge-neutral">{{ __('app.draft') }}</span>
                @endif
            </div>

            <div class="flex items-center gap-2 mb-4">
                <div class="flex-1 px-3 py-2 bg-midnight-950 rounded-lg">
                    <p class="text-xs text-midnight-500 mb-1">{{ __('app.public_url') }}</p>
            ...
            <div class="flex items-center gap-2 pt-4 border-t border-midnight-800">
                ...
                <form action="{{ route('status-pages.destroy', $page) }}" method="POST" 
                      onsubmit="return confirm('{{ __('app.delete_confirm_status_page') }}')">
                    @csrf
                    @method('DELETE')
