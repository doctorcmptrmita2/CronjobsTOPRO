<x-app-layout>
    <x-slot name="title">Uptime Monitoring</x-slot>

    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-midnight-50">Uptime Monitoring</h1>
                <p class="text-sm text-midnight-400 mt-1">Monitor your endpoints and get alerted when they go down</p>
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <div class="stat-card">
            <div class="flex items-center justify-between">
                <div>
                    <p class="stat-card-label">Down</p>
        ...
        <div class="stat-card">
            <div class="flex items-center justify-between">
                <div>
                    <p class="stat-card-label">Avg Uptime (24h)</p>
    ...
    <div class="card">
        <div class="px-6 py-4 border-b border-midnight-800">
            <h2 class="text-lg font-semibold text-midnight-50">Monitors</h2>
        </div>
        ...
        <div class="px-6 py-12 text-center">
            ...
            <h3 class="text-lg font-semibold text-midnight-200 mb-2">No uptime checks yet</h3>
            <p class="text-midnight-500 mb-6">Create your first check to start monitoring your endpoints.</p>
            <a href="{{ route('uptime.create') }}" class="btn-primary">
                <svg class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                </svg>
            ...
            <div class="p-6 hover:bg-midnight-800/30 transition-colors">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-6">
                        ...
                        <div class="text-center">
                            ...
                            <span class="text-xs text-midnight-500">Status</span>
                        </div>
                        ...
                        <div class="text-center w-20">
                            ...
                            @endif
                            <span class="text-xs text-midnight-500">Response</span>
                        </div>
                        ...
                        <div class="text-center w-20">
                            ...
                            <span class="text-xs text-midnight-500">Uptime</span>
                        </div>
                        ...
                        <div class="text-center w-20">
                            ...
                            <span class="text-xs text-midnight-500">Interval</span>
                        </div>
                        ...
                        <div class="flex items-center gap-2">
                            <form action="{{ route('uptime.toggle', $check) }}" method="POST" class="inline">
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="p-2 text-midnight-400 hover:text-midnight-100 hover:bg-midnight-800 rounded-lg transition-colors" title="{{ $check->is_active ? 'Pause' : 'Activate' }}">
                                    @if($check->is_active)
                            ...
                            <a href="{{ route('uptime.edit', $check) }}" class="p-2 text-midnight-400 hover:text-midnight-100 hover:bg-midnight-800 rounded-lg transition-colors" title="Edit">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <form action="{{ route('uptime.run-now', $check) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" class="p-2 text-midnight-400 hover:text-midnight-100 hover:bg-midnight-800 rounded-lg transition-colors" title="Run Now">
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="mt-4 flex items-center gap-0.5">
                    ...
                    <div class="flex-1 h-2 rounded-sm {{ $run->is_up ? 'bg-emerald-500' : 'bg-red-500' }}" 
                         title="{{ $run->checked_at->format('M d, H:i') }} - {{ $run->is_up ? 'Up' : 'Down' }} {{ $run->response_time_ms ? '(' . $run->response_time_ms . 'ms)' : '' }}"></div>
                    @endforeach
